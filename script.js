...
function openShareModal() {
  ...
  try {
    ...
    const url     = `${location.origin}${location.pathname}#bloom=${encoded}`;
    el.shareLinkIn.value = url;
  } catch(e) {
    el.shareLinkIn.value = `${location.origin}${location.pathname}`;
  }
...
async function downloadBloom() {
  ...
  const storyStage = document.getElementById('storyCanvas-stage');
  const bgColor    = BACKGROUNDS.find(b => b.id === state.bgId)?.color || '#F7F4EF';

  try {
    const canvas = await html2canvas(storyStage || el.stage, {
      scale:           1,
      useCORS:         true,
      allowTaint:      true,
    ...
    link.download = 'karwaa-bloom.png';
    link.href     = canvas.toDataURL('image/png', 1.0);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    showToast('✦ Downloaded! Check your folder.');
    setTimeout(() => el.dlModal.classList.add('open'), 600);

  } catch(err) {
    console.error('Download failed:', err);
    showToast('Download failed — try on desktop or Chrome');
  }
}
